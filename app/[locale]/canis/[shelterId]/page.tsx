...
import { BadgeCheck, Building2, ChevronLeft, ExternalLink, HandCoins, Mail, MapPin, MessageSquareText, PawPrint, Phone } from "lucide-react";
      ...
      <section className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <div className="rounded-3xl border border-border/20 bg-card p-6">
            ...
            {eligibility.canReview ? (
              ...
              <p className="mt-4 rounded-2xl border border-border/30 bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
                {!user ? copy.loginToReview : copy.notAdopterReview}
              </p>
            )}
        ...
        <aside className="space-y-6 lg:col-span-4">
          ...
          <div className="rounded-3xl border border-border/20 bg-card p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">{copy.donateText}</p>

            {!user ? (
              <Link
                href={`/${locale}/auth/login?next=/canis/${shelter.id}`}
            ...
            ) : !shelter.iban && !shelter.mbway && !shelter.donation_link ? (
              <p className="mt-4 rounded-2xl bg-muted/40 px-4 py-3 text-xs text-muted-foreground">
                {copy.donateNoInfo}
              </p>
            ) : (
              <ul className="mt-4 space-y-3 text-sm">
                {shelter.donation_message && (
                  <li className="rounded-xl bg-muted/40 px-3 py-2 text-xs italic text-muted-foreground">
                    {shelter.donation_message}
                  </li>
                )}
                {shelter.iban && (
                  <li>
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{copy.donateIban}</p>
                    <p className="break-all font-mono text-xs">{shelter.iban}</p>
                  </li>
                )}
                {shelter.mbway && (
                  <li>
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{copy.donateMbway}</p>
                    <p className="font-mono text-xs">{shelter.mbway}</p>
                  </li>
                )}
