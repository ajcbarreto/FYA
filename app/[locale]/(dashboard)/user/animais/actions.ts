...
async function ensureUserOwnsAnimal(
  ...
  if (!animal || animal.owner_profile_id !== userId) return null;
  return animal;
}

export async function createUserAnimal(formData: FormData) {
  const locale = getLocaleFromForm(formData);
  const input = parseAnimalInput(formData);

  if (!input) {
    redirect(`/${locale}/user/animais/novo?error=invalid_data`);
  }
  ...
  if (role !== "user") {
    redirect(`/${locale}/user/animais?error=not_authorized`);
  }

  const { data: created, error } = await supabase
    .from("animais")
    .insert({ ...input, owner_profile_id: user.id, canil_id: null })
    .select("id")
    .maybeSingle();

  if (error || !created) {
    console.error("[createUserAnimal] error:", error?.message);
    redirect(`/${locale}/user/animais/novo?error=save_failed`);
  }

  revalidatePath(`/${locale}/user/animais`);
  redirect(`/${locale}/user/animais/${created.id}?success=created`);
}

export async function updateUserAnimal(formData: FormData) {
  const locale = getLocaleFromForm(formData);
...
export async function deleteUserAnimal(formData: FormData) {
  const locale = getLocaleFromForm(formData);
  const animalId = String(formData.get("animalId") ?? "").trim();
  ...
  if (error) {
    redirect(`/${locale}/user/animais?error=delete_failed`);
  }

  revalidatePath(`/${locale}/user/animais`);
  redirect(`/${locale}/user/animais?success=animal_deleted`);
}

export async function updateUserAnimalStatus(formData: FormData) {
  const locale = getLocaleFromForm(formData);
  const animalId = String(formData.get("animalId") ?? "").trim();
  const status = String(formData.get("status") ?? "").trim().toLowerCase();

  if (!animalId || !ALLOWED_STATUS.includes(status)) {
    redirect(`/${locale}/user/animais?error=invalid_status`);
  }
  ...
  const ownership = await ensureUserOwnsAnimal(supabase, user.id, animalId);
  if (!ownership) redirect(`/${locale}/user/animais?error=not_authorized`);
  ...
  const { error } = await supabase
    ...
    .eq("id", animalId)
    .eq("owner_profile_id", user.id);
  if (error) redirect(`/${locale}/user/animais?error=save_failed`);

  revalidatePath(`/${locale}/user/animais`);
  redirect(`/${locale}/user/animais?success=updated`);
}
...
export async function uploadUserAnimalPhoto(formData: FormData) {
  ...
  if (!user) redirect(`/${locale}/auth/login?next=/user/animais/${animalId}`);
...
export async function setPrimaryUserAnimalPhoto(formData: FormData) {
  const locale = getLocaleFromForm(formData);
  const animalId = String(formData.get("animalId") ?? "").trim();
  const photoId = String(formData.get("photoId") ?? "").trim();
  const redirectBase = `/${locale}/user/animais/${animalId}`;

  if (!animalId || !photoId) redirect(`${redirectBase}?error=invalid_data`);
  ...
  const { error } = await supabase
    ...
    .eq("animal_id", animalId)
    .eq("id", photoId);
  if (error) redirect(`${redirectBase}?error=update_failed`);

  revalidatePath(redirectBase);
  redirect(`${redirectBase}?success=primary_set`);
}

export async function deleteUserAnimalPhoto(formData: FormData) {
  const locale = getLocaleFromForm(formData);
  const animalId = String(formData.get("animalId") ?? "").trim();
  const photoId = String(formData.get("photoId") ?? "").trim();
  const redirectBase = `/${locale}/user/animais/${animalId}`;
  ...
  const { data: photo } = await supabase
    ...
    .eq("animal_id", animalId)
    .maybeSingle();
  if (!photo) redirect(`${redirectBase}?error=photo_not_found`);
  ...
  if (error) redirect(`${redirectBase}?error=delete_failed`);
